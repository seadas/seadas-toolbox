...
def get_l1info_with_geofile(l1b_filename, geo_filename):
    """
...
def get_l1info(l1b_filename):
    """
    Returns l1info output, which is obtained by running l1info
    via subprocess.
    """
    output = None
    cmd = ["l1info", l1b_filename]
    direct_output = get_command_output(cmd)
    if direct_output:
        output = direct_output.splitlines()
    return output
...
def get_timepart_l1b(l1b_filename, viirs_sensor):
  if (viirs_sensor == None or l1b_filename == None):
    return None
  #print(viirs_sensor)
  filename_split1 = l1b_filename.split(viirs_sensor + ".")
  if (len(filename_split1) == 2):
    filename_part2 = filename_split1[1]

    filename_split2 = filename_part2.split(".L1B.nc")
    if (len(filename_split2) == 2):
      timepart = filename_split2[0]
      return timepart
...
def get_viirs_sensor_from_url(l1a_filename_url):
  ...
  filename_split = l1a_filename_url.split("PACE_OCI")
  if (len(filename_split) == 2):
    return "PACE_OCI"
...
def get_l1b_filename_from_url(l1b_filename_url, viirs_sensor):
  filename_split = l1b_filename_url.split(viirs_sensor)
  if (len(filename_split) == 2):
    return  viirs_sensor + filename_split[1]
...
# verbose=1
  ...
  par_lines.append("area_weighting=1")
    ...
    with open(scene_file_list) as file:
      ...
      for l1a_filename_entry in filelines:
        ...
        if (viirs_sensor == None):
          print("WARNING! not a supported sensor in this program " + viirs_sensor)
          continue

#   if (viirs_sensor == "JPSS1_VIIRS" or viirs_sensor == "JPSS2_VIIRS" or viirs_sensor == "SNPP_VIIRS"):
#     l2prod = "rhos_489,rhos_556,rhos_667,rhot_489,rhot_556,rhot_667,senz,solz"
#     l2prod = "rhos_489,rhos_556,rhos_667,rhos_868,rhos_1238,rhos_1604,rhos_2258,rhot_489,rhot_556,rhot_667,senz,solz"
#     #print(l3bprod)
#   elif (viirs_sensor == "SNPP_VIIRS"):
...
#     return None
        ...
        if (viirs_sensor == "JPSS1_VIIRS" or viirs_sensor == "JPSS2_VIIRS" or viirs_sensor == "SNPP_VIIRS"):
          ...
          if (l1a_filename == None):
            print("WARNING!  level-1A filename not found in URL")
            continue
          print(l1a_filename)
          ...
          l1b_filename =  get_l1b_filename_from_url(l1a_filename_url, viirs_sensor)
          if (l1b_filename == None):
            print("WARNING!  level-1B filename not found in URL")
            continue
          print(l1b_filename)

          timepart = get_timepart_l1b(l1b_filename, viirs_sensor)
          if (timepart == None):
            print("WARNING! could not derived timepart from " + l1b_filename)
            continue
          print(timepart)
        else:
          print ("unknown viirs sensor " + viirs_sensor)
    ...
    #    print("geo_filename=" + geo_filename)
        ...
        if (viirs_sensor == "JPSS1_VIIRS" or viirs_sensor == "JPSS2_VIIRS" or viirs_sensor == "SNPP_VIIRS"):
          ...
          if (need_l1b_file):
            download_obdaac_file(l1b_filename)
            if (os.path.isfile(l1b_filename) != True):
              print("ERROR: Failed to download Level-1B file " + l1b_filename)
              continue
        ...
        if (need_l2_file):
          ...
          if (viirs_sensor == "JPSS1_VIIRS" or viirs_sensor == "JPSS2_VIIRS" or viirs_sensor == "SNPP_VIIRS"):
            lines = get_l1info_with_geofile(l1b_filename, geo_filename)
#             print(lines)
            if (lines == None):
...
#           print("End_Date=" + end_date)
          ...
          if (viirs_sensor == "JPSS1_VIIRS" or viirs_sensor == "JPSS2_VIIRS" or viirs_sensor == "SNPP_VIIRS"):
            ...
            run_l2gen(l1b_filename, None, l2_filename, viirs_sensor, daynight, sunzen_curr, satzen_curr)
...
# scene_file_list_front = frontside_files_list
...
viirs_sensor, timepart = create_scene_files(frontside_files_list, binned_list_file_front, binned_resolution, FRONTSIDE, sunzen_north, sunzen, sunzen_south, satzen_north, satzen, satzen_south )
