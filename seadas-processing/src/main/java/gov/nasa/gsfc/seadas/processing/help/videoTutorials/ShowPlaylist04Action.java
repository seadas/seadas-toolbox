...
@ActionReference(
        path = MenuLocationUtils.SEADAS_VIDEOS_LOCATION + "/Science Processors",
        position = 0,
        separatorAfter = 1)
@NbBundle.Messages({
        "CTL_ShowPlaylist04Action_DisplayName=PLAYLIST: Science Processors",
        "CTL_ShowPlaylist04Action_ShortDescription=Opens YouTube playlist containing SeaDAS tutorial videos"
})

public class ShowPlaylist04Action extends AbstractSnapAction implements  LookupListener, Presenter.Menu {

    private static final String DEFAULT_PAGE_URL = "https://youtube.com/playlist?list=PLf60TttfDm33j85VSBc3sTDcAjtiZANjB";

    private final Lookup lookup;

    public ShowPlaylist04Action() {
        this(null);
    }

    public ShowPlaylist04Action(Lookup lookup) {
        putValue(ACTION_COMMAND_KEY, getClass().getName());
        putValue(SELECTED_KEY, false);
        putValue(NAME, Bundle.CTL_ShowPlaylist04Action_DisplayName());
        putValue(SHORT_DESCRIPTION, Bundle.CTL_ShowPlaylist04Action_ShortDescription());
    ...
    public void actionPerformed(ActionEvent event) {
        DesktopHelper.browse(Config.instance().preferences().get("seadas.playlist04", DEFAULT_PAGE_URL));
        updateEnabledState();
    }
