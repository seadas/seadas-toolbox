...
public final class PanoplyDumpTopComponent extends TopComponent implements LookupListener {
    ...
    private org.openide.util.Lookup.Result<MetadataElement> metaSel;
    ...
    @Override public void componentClosed() {
        if (metaSel != null) metaSel.removeLookupListener(this);
        metaSel = null;
    }
    ...
    @Override public void resultChanged(LookupEvent ev) {
        ...
        textArea.setText(buildDumpFrom(el));
    ...
    private static String buildDumpFrom(MetadataElement varElem) {
        var attrs = new ArrayList<>(Arrays.asList(varElem.getAttributes()));
        attrs.sort(Comparator.comparingInt(a -> {
            var n = a.getName();
            int i = n.lastIndexOf('\u200B');
            if (i < 0) return Integer.MAX_VALUE;
            try { return Integer.parseInt(n.substring(i + 1)); } catch (NumberFormatException ex) { return Integer.MAX_VALUE; }
        }));
